...
# Train model
model = XGBRegressor(n_estimators=200, max_depth=5, eta=0.1, subsample=0.7, colsample_bytree=1.0)
...
with ExcelWriter('XGBoost01.xlsx') as writer:
    df11.to_excel(writer, sheet_name='XGBoost', startrow=3, startcol=2, header=False, index=False)
    df12.to_excel(writer, sheet_name='XGBoost', startrow=3, startcol=3, header=False, index=False)
